...
FAOSTAT is the largest agricultural database in the world. It contains data from land productivity to agricultural production and trade dating back from 1960 to the most recent available data. Detailed information on meta data, methods and standards can be found on the official website of FAOSTAT (\url{http://faostat.fao.org/}) and the Statistics Division (ESS) (\url{http://www.fao.org/economic/ess/en/}).

In order to access to an indicator in FAOSTAT using the API, at least two pieces of information need to be provided: domain and element codes. They are defined as:
